/*******************************************************************************
  ...
  Libraries: NavigationOnArduino,
                               ArduinoQueue:
https://github.com/EinarArnason/ArduinoQueue, Display:
...
void loop() {
  ...
  for (int i = 0; i < osmMap.getNumberOfWaypoints(); i++) {
    NavPoint current = osmMap.getWaypoint(i);
  ...
  // draw ways
  for (int w = 0; w < osmMap.getNumberOfWays(); w++) {
    for (int p = 0; p < osmMap.getNumberOfPointsInWay(w) - 1; p++) {
      NavPoint p1 = osmMap.getWaypointFromWay(w, p);
      NavPoint p2 = osmMap.getWaypointFromWay(w, p + 1);
