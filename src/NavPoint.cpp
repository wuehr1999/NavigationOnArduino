...
float NavPoint::getLatitude() const { return latitude; }

void NavPoint::setLatitude(float latitude) { this->latitude = latitude; }

float NavPoint::getLongitude() const { return longitude; }

void NavPoint::setLongitude(float longitude) { this->longitude = longitude; }

float NavPoint::calculateDistance(NavPoint point) {
  float la1 = latitude;
  float lo1 = longitude;
  float la2 = point.getLatitude();
  float lo2 = point.getLongitude();
  ...
  float a =
      ...
      cos(latitude1R) * cos(latitude2R) * sin(dlong / 2.0) * sin(dlong / 2.0);
...
float NavPoint::calculateDistanceFromTrack(NavPoint trackpoint1,
                                           NavPoint trackpoint2) {
  float dist = trackpoint1.calculateDistance(*this) / 6371000.0;

  float bearing1 = (trackpoint1.calculateBearing(*this) + 180) * M_PI / 180.0;

  float bearing2 =
      (trackpoint1.calculateBearing(trackpoint2) + 180) * M_PI / 180.0;
...
void NavPoint::xyPixelsFromStartPoint(NavPoint startPoint, int *x, int *y,
                                      float metersPerPixel) {
  float distance = startPoint.calculateDistance(NavPoint(latitude, longitude));
  float bearing =
      startPoint.calculateBearing(NavPoint(latitude, longitude)) - 90.0;
